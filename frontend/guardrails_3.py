...
def apply_universal_standardization():
    ...
    # We will replace lines 476 to 588 which handles the `motion.div` Image Inspection Area
    old_inspection_area = """               <motion.div 
                 layout
    ...
    new_inspection_area = """               <motion.div 
                 layout
    ...
    if old_inspection_area in content:
        content = content.replace(old_inspection_area, new_inspection_area)
        with open(file, 'w', encoding='utf-8') as f:
            f.write(content)
        print("Successfully applied universal standardization.")
    else:
        print("Could not find the target code block to replace.")

if __name__ == '__main__':
    apply_universal_standardization()
