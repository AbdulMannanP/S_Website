...
footer_html = """  <!-- ── Footer ───────────────────────────────────────── -->
  <footer id="footer" class="bg-[#07070a] border-t border-[#c9a96e]/10 py-16 px-6 text-center">
    ...
    <!-- Phone + Address -->
    <p class="text-white/50 text-sm mb-1" dir="ltr"><a href="tel:+9669288004450" class="hover:text-[#c9a96e] transition-colors">+966 92 880 04450</a> — Al Qurayyat, Al Jawf, KSA</p>
    ...
    <!-- Logo -->
    <img src="https://ik.imagekit.io/de7qvcvqv/images/logo.png?updatedAt=1779608592778" loading="lazy" alt="Saeed Furniture"
         width="120" height="40"
...
def update_file(filepath):
    ...
    # We want to replace everything from the start of the footer to the end of </footer>
    # Most files have: <footer ...> ... </footer>
    
    # For index.html, there's also the Ask on WhatsApp button that we might want to remove if we are replacing the footer, 
    # but let's just replace the <footer...>...</footer> block.
    
    pattern = re.compile(r'<!-- ── Footer [─]+ -->\s*<footer.*?</footer>', re.DOTALL | re.IGNORECASE)
    
    # Also handle <!-- ===================== UNIFIED GLOBAL FOOTER ===================== --> in select.html
    pattern2 = re.compile(r'<!-- =+ UNIFIED GLOBAL FOOTER =+ -->\s*<footer.*?</footer>', re.DOTALL | re.IGNORECASE)
    
    # Also handle <!-- ── Footer ────────────────────────────── --> in majlis.html
    pattern3 = re.compile(r'<footer.*?id="footer".*?</footer>', re.DOTALL | re.IGNORECASE)
    
    if pattern.search(content):
        content = pattern.sub(footer_html, content)
    elif pattern2.search(content):
        content = pattern2.sub(footer_html, content)
    elif pattern3.search(content):
        content = pattern3.sub(footer_html, content)
    else:
        # if there is just a footer tag
        pattern4 = re.compile(r'<footer.*?</footer>', re.DOTALL | re.IGNORECASE)
        if pattern4.search(content):
            content = pattern4.sub(footer_html, content)
        else:
...
html_files = [f for f in os.listdir('.') if f.endswith('.html') and f not in ['auth.html', 'portfolio.html']]
