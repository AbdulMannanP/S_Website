...
# Remove the JS click handler for the polygons:
# poly.addEventListener('click', () => { ... });
# It starts at: poly.addEventListener('click', () => {
# and ends at the next block or at the end of the forEach.
# Let's use regex to remove it.
click_regex = re.compile(r"poly\.addEventListener\('click', \(\) => \{.*?\}\);\s*", re.DOTALL)
html = click_regex.sub('', html)

# Remove all remaining OLD nodes. They are bounded by:
# <!-- ── NODE: ... Showroom ────────... -->
# up to the end of </main>
node_start = html.find('<!-- ── NODE: Kitchen Showroom')
if node_start == -1:
    node_start = html.find('<!-- ── NODE:')
main_end = html.rfind('</main>')
if node_start != -1 and main_end != -1 and node_start < main_end:
    html = html[:node_start] + '\n  ' + html[main_end:]
...
print("Re-generated 5 pages successfully.")
