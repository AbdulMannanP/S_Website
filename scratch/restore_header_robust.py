...
for filepath in html_files:
    ...
    # Find the start of the unified header
    start_header = content.find('<header class="fixed top-0 inset-x-0 z-[150] bg-stone-950/80 backdrop-blur-xl border-b border-white/5 transition-all duration-300">')
    if start_header != -1:
        # Find the end of the mobile nav. The mobile nav looks like this:
        ...
        mobile_nav_str = '<a href="/contact.html" class="block text-white text-sm tracking-widest uppercase hover:text-amber-400">Contact</a>\n    </div>'
        # Alternatively, find </header> and then the next </div>
        end_header = content.find('</header>', start_header)
        end_mobile_nav = content.find('</div>', content.find('<div id="mobile-nav"', end_header))
        
        if end_mobile_nav != -1:
            content = content[:start_header] + transparent_header + content[end_mobile_nav+6:]
